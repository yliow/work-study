...
a0,b0 = 0.5, sqrt(3)/2
X = POINT(x=a0, y=b0, label='$z = 1/2 + i\sqrt{3}/2$',
          anchor='south west')
p += str(X)
...
a1,b1 = 0.5, -sqrt(3)/2
X = POINT(x=a1, y=b1, label="$z' = 1/2 - i\sqrt{3}/2$",
          anchor='north west')
p += str(X)

p += Line(points=[(0,0), (a0, b0)], linestyle='dashed')
p += Line(points=[(0,0), (a1, b1)], linestyle='dashed')
