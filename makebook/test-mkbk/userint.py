...
def select_process(found_notes):
    ...
    if choice == 'q':
        ...
        for i in choice:
            #print(i)
            if i.isnumeric() and 0 <= int(i) < len(found_notes) and i not in selected_notes:
                print(i not in selected_notes)
...
#use 0 when handling errors to keep other options 1 through 5 from being selected
def menu_loop():
    greeting()
    selection = 0
    while 0 <= selection < 5:
