...
def extraction_to_file(ex_dest, note, ex_content, number):
  ...
  #assign file name and string for exercise main.tex, update main.tex "
  ...
  try:
    with open(question, 'w') as x:
      x.write("\\tinysidebar{\\debug{exercises/{%s/question.tex}}}" % ex_file)
...
for chap in chap_list:
  ...
  new_note_dir = dir_ + '/temp/' + chap + '/'
  ex_dest = new_note_dir + 'exercises/'
  try:
    #check if note dir exists    
    if not os.path.exists(new_note_dir):
      print("making chapter %s dir at %s..." % (chap, new_note_dir))
      os.system("mkdir %s" % (new_note_dir))
    ...
    with open(note, 'r') as n:
      ...
      for line in n:
        ...
        if begin in line:
          ...
          if exercise != None:
            counter += 1
            note_acc.append("\\input{exercises/%s/main.tex}" % exercise )
          else:
    ...
    #write new file
    with open(new_note_dir + 'main.tex', 'w') as f:
