...
seem like arrays. I'll talk about the some of the
...
Note the address value stored in \textit{q} ... numerically speaking the
value of \textit{q} is actully the numerical value of \textit{p} plus 4.
when you add 1 to the address of an \textit{int} address --
IT'S NOT ONE!!! Why 4? Because \ldots{} \textbf{4 comes
from} \textit{\textbf{sizeof(int)}}.
...
Now you might be wondering: ``Wait \ldots{} \textit{p} points to \textit{x}
and I do see that the value of \textit{x} is 42. But what is the 3467 that
\textit{p + 1} points to?''.

\textit{p + 1} simply points to an integer (because \textit{p} is a pointer
to \textit{int}), i.e., the four bytes starting at \textit{p + 1} as an
...
The above applies to any type and not just \textit{int}. In general
suppose \textit{T} is a type (\textit{int} or a \textit{char} or a
\textit{double} or a \textit{bool} or what-have-you), and you have the
following:

\verb!T * p = &x;!

and suppose a value of type \textit{T} uses k bytes (which can be obtained
using \textit{sizeof(T)}), then numerically the address value of \textit{p +
1} is always the address value of \textit{p} plus \textit{k}. Also, if we
have
...
then \textit{q} is k smaller than \textit{p}.

\textbf{Remember ...}

\begin{itemize}
\item if \textit{p} is an \textit{int} pointer (and if you're
  using a 32-bit machine), then \textit{p + 1} does \textbf{not} mean
  ``next address value after \textit{p}''. Rather, it means address of the
  next \textbf{integer} after the \textbf{integer} \textit{p} points to.
\item If \textit{q} is a pointer that points to a \textit{double}, then \textit{q
  + 1} is the address of the next \textbf{double} after the
  \textbf{double} \textit{q} points to.
\item If \textit{r} is a pointer that points to a character, then \textit{r + 1}
...
However the \textit{sizeof(int)} can vary depending on your machine.
...
\begin{ex}
Think about this: Is \textit{(int) (p + 1)} the same as
(\textit{(int) p) + 1}?
\end{ex}

\begin{ex}
There are four pointer values (i.e., address values)
in the following code segment. The first one is \texttt{p}. As for the
rest:
...
How much larger is \texttt{q} when compared with \texttt{p}? Verify it
...
(Why 31? It's 1 less than 32, the number of bits for a
32-bit machine.) There is another integer type called the long integer.
The formal name is \texttt{long} or \texttt{long int}. Example:
...
\begin{ex}
There is yet another integer type called the long long
integer. The formal name is \texttt{long long} or \texttt{long long
int}. Example:
...
assuming that \texttt{\textbf{sizeof(int)}}\textbf{ is 4}. (If on your
machine, the \texttt{sizeof(int)} is 8, then the address of \texttt{y} is 8
smaller.)

\textbf{Caveat:} Note that that difference between \texttt{\&x} and
\texttt{\&y} can be more than 4 on your computer, but it cannot be less
than \texttt{4}. The point is that an integer takes up \texttt{sizeof(int)}
...
-- 4 = 8 since the \texttt{sizeof(int)} is 4.
...
(Recall: you might want to use \texttt{unsigned long long}.) AHA \ldots{}
...
bytes between the memory of \texttt{x} and the memory of \texttt{y}, for
\texttt{q} to point to \texttt{y}, I only need \texttt{q = p - 1}. On my
Windows machine, there is a padding of 8 bytes, which means that for my
Windows machine, I would need to do \texttt{q = p - 1 - 2}, i.e., I have
to subtract 2 more because of the 8-byte padding.

So if the address of \texttt{x} is 1000 (and assuming there is no extra
padding), then the address of \texttt{y} is 996 and the address of
\texttt{z} is 992.
...
for \texttt{y} is immediately after the value for \texttt{x} and the value
for \texttt{z} is immediately after \texttt{y}. If the address of the value
of \texttt{x} is 4800000, what is the address of the value for \texttt{y}?
What is the address of \texttt{z}?
\end{ex}
...
\begin{itemize}
\item the values in the array are laid out in \textbf{ascending} address
  values and
\item there are \textbf{no gaps} in the layout of the memory between values
  of the array (there's no memory padding -- see previous section)
\end{itemize}

Since \texttt{sizeof(int)} is 4,

\textbf{ the address of }\texttt{\textbf{x[i + 1]}}\textbf{ is the address }

\textbf{ of }\texttt{\textbf{x[i]}}\textbf{ plus 4}

Note in particular the big difference between a bunch of \texttt{int}
...
since \texttt{sizeof(double)} is 8, the addresses of \texttt{y[0]},
\texttt{y[1]}, \texttt{y[2]}, \texttt{y[3]} go up by a step of 8.

In general if \texttt{z} is an array of type \texttt{T} values, then

the address of \texttt{z[i + 1]} is the address

of \texttt{z[i]} plus \texttt{sizeof(T)}

This is great because

if \texttt{p} points to \texttt{z[i]}, then \texttt{p + 1} points to
\texttt{z[i + 1]}

and

if \texttt{p} points to \texttt{z[0]}, then \texttt{p + i} points to
\texttt{z[i]} and so \texttt{*(p + i)} is the same as
\texttt{z[i]}
...
You will see an address!!! That tells you that the array \texttt{x} is
somehow an address!!!
...
This tells you that if you think of \texttt{x} as a pointer, then
\texttt{\textbf{x}}\textbf{ is the address of the first value }of your
array of value.

So what's happening?!?

Whenever you use \texttt{x} as though it's a value, the
compiler will actually replace \texttt{x} with \texttt{\&x[0]}, i.e.,
the address of the first value of of the array:

\texttt{\textbf{x}}\textbf{ is the same as }\texttt{\textbf{\&x[0]}}

You also know that you can perform pointer arithmetic. Since \texttt{x}
...
\texttt{x[0]}, \texttt{x[1]}, \texttt{x[2]}, \texttt{x[3]},
\texttt{x[4]}

But since \texttt{x}, \texttt{x + 1}, \texttt{x + 2}, \texttt{x + 3}, \texttt{x +
4} points to the above values, we conclude that you can also access the
values of the array using:

\texttt{*x}, \texttt{*(x + 1)}, \texttt{*(x + 2)}, \texttt{*(x + 3)}, \texttt{*(x
+ 4)}
...
In fact given \textbf{any} pointer \texttt{p} (not just an array)
and integer \texttt{i}, in C/C++,

\texttt{\textbf{p[i] }}\textbf{is the same as }\texttt{\textbf{*(p +
i)}}
...
\begin{ex}
You are given that the address of the first value of
the following array

\verb!int x[100];!

is 72008004. Assuming that each \texttt{int} value takes up 4 bytes of
memory, what is the address of the value \texttt{x[3]}? What is the
memory address of \texttt{x[56]}?
\end{ex}
...
Assuming that an \texttt{int} takes up 4 bytes and a \texttt{double} takes
up 8 byte, the address of \texttt{x} is 120004000, what is the address of
\texttt{a[3]} and \texttt{b[20]}? (Assume that there is no gaps in
...
``hey heap \ldots{} gimme a chunk of an \texttt{int} array of \texttt{s}
values''

Note that \texttt{s} can be a variable!

Mr Heap then goes through its memory and looks for an unused
\textbf{contiguous} chunk of memory for an array of \texttt{s} \texttt{int}
values, marks this chunk as ``in use'' (i.e. not free) and
\textbf{returns the address of the }\texttt{\textbf{first}}\textbf{
...
p += Rect(8.5, 7.5, 8.5, 7.5, label='Free store', linewidth=0)
p += Rect(7, 0, 15, 4)
p += Rect(8.5, 3.5, 10.5, 4.5, label='512')
p += Rect(10.5, 3.5, 12.5, 4.5, label='4')
p += Rect(12.5, 3.5, 14.5, 4.5, label='-75')

p += Rect(10.5, 8, 14.5, 13.5, linewidth=0.1, linecolor='red', innersep=0.1, label='The heap will mark these 3 integers as "in use".', aling='c')

p += Line(points=[(2, 1.5), (7.5, 3.5)], linewidth=0.15, endstyle='>')
p += Line(points=[(12.5, 8), (13.5, 4.5)], linewidth=0.15, linecolor='red', endstyle='>')
...
This will release the whole array of \texttt{int} values at address
\texttt{x} back to the heap so that the heap can reuse this memory for
...
\texttt{x} points to), not the whole array, back to the heap. The problem
...
Most programmers use \texttt{x[i]} since we want to think of \texttt{x}
as an array (although technically it's a pointer.) The
...
You have to be clear about the three because there are similarities and
...
\begin{itemize}
\item Both static arrays and dynamic arrays are arrays (i.e., linear
...
\begin{itemize}
\item Both types of arrays are homogeneous, i.e., all the values in an array
  have the same type.
\end{itemize}
...
in the heap, then \texttt{x[i]} is just the value at location \texttt{x
+ i}. You can have \texttt{x[42]}, \texttt{x[-5]}, etc.!!! You can
access the value at address \texttt{x + 42} and \texttt{x + (-5)} as long as
...
The reason (as stated above) is because in the above \texttt{x} is the
same as \texttt{\&x[0]}. So the above make \texttt{x} like a pointer
...
So what's \texttt{x} if \texttt{x} is an array since it's not a pointer? You
can view \texttt{x} as a constant pointer. In other words, you can think
of \texttt{x} as a pointer but the address of \texttt{x} cannot be changed.
...
If you want to make \texttt{y} as close to \texttt{x} as possible, you can
do this:
...
Another difference is in the behavior of the \texttt{sizeof()} function.
Try this:
...
The \texttt{sizeof()} on an array returns the size of memory used for the
array of values. The \texttt{sizeof()} on a pointer returns the size of
...
\textbf{constant} pointer? Let's try an experiment:
...
\textbf{An array parameter is actually a non-constant pointer}


\newpage\EMPHASIZE{Default values}

There's another thing that we should investigate: What
about default values? Let's try this:
...
There \textbf{is} one case where a pointer can be initialized to
point to values and only for certain compilers. Therefore you
shouldn't use the following in real code:

\verb!const char * x = "hello world";!

Again. Don't use this in real code. It's
...
But what if I want to compute \texttt{x[4] + x[5] + x[6]}? In
...
Actually it's \textbf{not necessary at all (if you know
pointers)!}
...
to compute \texttt{x[4] + x[5] + x[6]}. In this case, the
...
\begin{ex}
Rewrite the following program so that the prototype of the max
function looks like this:

\verb!int max(int x[], int size);!

Modify the test code in \texttt{main()} so that the same result is
produced.
...
You \textbf{can subtract pointers}.

The result is this: If \texttt{p} and \texttt{q} each points to an integer
value in \textit{the same array} of integers, then
\texttt{q - p}
gives you the number of integers from the one that \texttt{p} points to
up to (but not including) the one that \texttt{q} points to; this is
negative if \texttt{q} points to an integer that appears before the one
that \texttt{p} points to.

This is like subtracting indexes. Think of \texttt{p} as address-of \texttt{x[i]} and
\texttt{q} as address-of \texttt{x[j]}, i.e., \texttt{p} = \verb!&x[i]! and \texttt{q} = \verb!&x[j]!, then
\verb!q - p! gives you \verb!j - i!, the number of integers starting at
\texttt{x[i]} and ending \textit{before} \texttt{x[j]}; this is negative if
\verb!j < i!.

Note that in memory, an integer can be 4 bytes. Then the difference of
\verb!q - p! is NOT the difference of bytes, but the difference of
integers.

Here's an important use of pointer subtraction.

If you try to write a pointer version of binary search:
...
exercise on the \texttt{array\_append()} function.
...
\begin{ex}
Write a program that implements a stack of integers
...
\begin{ex}
Write a program that implements a queue of integers
...
\begin{ex}
Write a program that implements a set of integers
...
If I do \verb!int * x = new int[10];! x points to a 1D array of size
10.
...
# John at top, Sue at bottom-left, Tom at bottom-right
john_x, john_y = 4, 4
sue_x, sue_y = 1, 1
tom_x, tom_y = 7, 1

# Draw nodes as circles with labels
p += Rect(john_x, john_y, john_x, john_y, label='John')
p += Rect(sue_x, sue_y, sue_x, sue_y, label='Sue')
p += Rect(tom_x, tom_y, tom_x, tom_y, label='Tom')

# Draw circles around nodes
p += ellipse(john_x-0.5, john_y-0.3, john_x+0.5, john_y+0.3, linewidth=0.1)
p += ellipse(sue_x-0.5, sue_y-0.3, sue_x+0.5, sue_y+0.3, linewidth=0.1)
p += ellipse(tom_x-0.5, tom_y-0.3, tom_x+0.5, tom_y+0.3, linewidth=0.1)

# John -> Sue (arrow going down-left)
p += Line(points=[(john_x-0.5, john_y-0.3), (sue_x+0.5, sue_y+0.3)], linewidth=0.1, endstyle='>')

# John -> Tom (arrow going down-right)
p += Line(points=[(john_x+0.5, john_y-0.3), (tom_x-0.5, tom_y+0.3)], linewidth=0.1, endstyle='>')

# Sue -> Tom (arrow going right)
p += Line(points=[(sue_x+0.6, sue_y), (tom_x-0.6, tom_y)], linewidth=0.1, endstyle='>')

# Tom -> John (arrow going up-left)
p += Line(points=[(tom_x-0.3, tom_y+0.4), (john_x+0.3, john_y-0.4)], linewidth=0.1, endstyle='>')
...
character arrays (containing the null character `\textbackslash 0'). So
...
\begin{ex}
Now, how can we save memory for the social media matrix? You
...
\begin{ex}
Path connectivity problem. Now add the following to
...
# John <-> Sue (bidirectional arrows)
p += Line(points=[(john_x-0.6, john_y-0.2), (sue_x+0.4, sue_y+0.4)], linewidth=0.1, endstyle='>')
p += Line(points=[(sue_x+0.6, sue_y+0.3), (john_x-0.4, john_y-0.4)], linewidth=0.1, endstyle='>')

# John <-> Tom (bidirectional arrows)
p += Line(points=[(john_x+0.6, john_y-0.2), (tom_x-0.4, tom_y+0.4)], linewidth=0.1, endstyle='>')
p += Line(points=[(tom_x-0.6, tom_y+0.3), (john_x+0.4, john_y-0.4)], linewidth=0.1, endstyle='>')

# Sue <-> Tom (bidirectional arrows)
p += Line(points=[(sue_x+0.6, sue_y+0.1), (tom_x-0.6, tom_y+0.1)], linewidth=0.1, endstyle='>')
p += Line(points=[(tom_x-0.6, tom_y-0.1), (sue_x+0.6, sue_y-0.1)], linewidth=0.1, endstyle='>')
...
# John -- Sue (line without arrows)
p += Line(points=[(john_x-0.5, john_y-0.3), (sue_x+0.5, sue_y+0.3)], linewidth=0.1)

# John -- Tom (line without arrows)
p += Line(points=[(john_x+0.5, john_y-0.3), (tom_x-0.5, tom_y+0.3)], linewidth=0.1)

# Sue -- Tom (line without arrows)
