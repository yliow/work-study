...
Note that you\textquotesingle re doing the \emph{same thing} to three
variables. There\textquotesingle s a C++ concept that allows you (more
or less) to say
...
It\textquotesingle s the same idea. Your instructor wanted to do
something to all the variables x\textsubscript{0}, x\textsubscript{1},
x\textsubscript{2}, and x\textsubscript{3. }But instead of saying ``Set
x\textsubscript{0 }to 42'', ``Set x\textsubscript{1 }to 42'', ``Set
x\textsubscript{2} to 42'', ``Set x\textsubscript{3 }to 42'', your math
instructor was smart enough to say:``For each x\textsubscript{i} with i
= 0, 1, 2, 3, we want to set x\textsubscript{i }be 42''. He/she is
basically using subscripts.

C++ supports the same way of applying operations through a bunch of
variables, and not just one.

That\textquotesingle s the concept of an \emph{\textbf{array}} of
values.
...
Although I\textquotesingle m calling \emph{a{[}0{]}}, \emph{a{[}1{]}},
and \emph{a{[}2{]}} the values into array \emph{a}, you can
\textbf{think of them as integer variables}. Therefore you
\emph{\textbf{do}} know how to use them.

You might say: ``Well what\textquotesingle s the point?
Can\textquotesingle t we just use three variables such as \emph{x},
\emph{y}, and \emph{z} instead of \emph{a{[}0{]}}, \emph{a{[}1{]}}, and
\emph{a{[}2{]}}???'' Don\textquotesingle t worry we\textquotesingle ll
come to that very soon.
...
should see two 1\textquotesingle s.)
...
Here\textquotesingle s an example from the previous section.
...
\vtop{\hbox{\strut  std::cout \textless\textless{} a{[}i{]}
...
Now let\textquotesingle s see how much time you'd need to modify the
\emph{std::cin} statement if you have not heard of arrays:
...
In the next few sections, I\textquotesingle ll show you how to apply
this principle in different scenarios.

One final thing before I show you different uses of the array.
It\textquotesingle s always a good idea not to hardcode constants,
...
There\textquotesingle s another way to avoid hardcoding the size of an
array. The function \emph{sizeof()} will tell you the mount of memory
...
char z = \textquotesingle a\textquotesingle;
...
\textless\textless{} sizeof(y) \textless\textless{}
\textquotesingle\textbackslash n\textquotesingle{}

\textless\textless{} sizeof(z) \textless\textless{}
\textquotesingle\textbackslash n\textquotesingle{}

\textless\textless{} sizeof(a) \textless\textless{}
\textquotesingle\textbackslash n\textquotesingle;
...
So here\textquotesingle s how you can determine the number of elements
...
\vtop{\hbox{\strut  std::cout \textless\textless{} max
\textless\textless{} std::endl;}\hbox{\strut \}}} \\
\end{longtable}

If you don\textquotesingle t see it, just unroll the for-loop.

Neat isn\textquotesingle t it?
...
\vtop{\hbox{\strut  std::cout \textless\textless{} min
\textless\textless{} std::endl;}\hbox{\strut \}}} \\
\end{longtable}
...
char target = \textquotesingle{} \textquotesingle;
...
std::cout \textless\textless{} found \textless\textless{} std::endl; \\
\end{longtable}
...
std::cout \textless\textless{} index \textless\textless{} std::endl; \\
\end{longtable}
...
It\textquotesingle s important to understand that
there\textquotesingle s no reason that you must scan the array from the
first index to the last or from the last to the first.
...
Here\textquotesingle s an ASCII problem. Prompt the user for two
integers for variables i and j (from 0 to 9). The program prints two
X\textquotesingle s at column i and j. For instance if the user enters 0
and 3, the program prints
...
Now your program\textquotesingle s logic should be like this:

print i spaces

print \textquotesingle X\textquotesingle{}
...
But now you need to go backwards ... and you can\textquotesingle t! The
print cursor can only go forward! So you really need to sort i and j.
...
But there\textquotesingle s another way to solve this problem. You can
create an array of characters, put the
\textquotesingle X\textquotesingle{} in the array of characters and then
print the array. Why would you do that? Because you have complete
control over where you want to put characters into the array!

declare array a of characters

set a{[}i{]} to \textquotesingle X\textquotesingle{}

set a{[}j{]} to \textquotesingle X\textquotesingle{}
...
\vtop{\hbox{\strut  a{[}i{]} = \textquotesingle{}
...
a{[}i{]} = \textquotesingle X\textquotesingle;

a{[}j{]} = \textquotesingle X\textquotesingle;
...
we don\textquotesingle t really care if i is greater than j.
...
2, 3, 4, \ldots, 10 instead of d = 2, 3, 4, \ldots, 100.
That\textquotesingle s a saving of 90\% \ldots{} !!!

But \ldots{} there\textquotesingle s \emph{yet} another way to speed up
the program: you only need to test n \% d for d running through
\emph{primes} up to the square root of n. Therefore for n = 101, you
only need to test n \% d for d = 2, 3, 5, 7. So from d = 2, 3, 4,
\ldots, 10 you go down to d = 2, 3, 5, 7, from 9 values of d to 4.
That\textquotesingle s about 50\% \ldots{} !

How would an array help?

One way is to compute an array of 1000 primes before you run your
program. Let\textquotesingle s say the array is called:
...
{[}Don\textquotesingle t recall any of the above? Quickly review your
previous notes on bubblesort!!! Now!!!{]}

Of course you know what\textquotesingle s going to happen next.

We\textquotesingle re going to sort values in an array!!! (cheers ...
clapping ... and wild whistling).

Let\textquotesingle s do this slowly. Using the above pseudocode but
with a, b, c, d replaced by a{[}0{]}, a{[}1{]}, a{[}2{]}, a{[}3{]}, we
have the following:
...
See that? What about Pass 3? Pass 3 looks like this (I\textquotesingle m
writing this as a for-loop just to see the pattern):
...
array. Let\textquotesingle s do a simple example before we talk about
code.
...
Here\textquotesingle s another way of doing it. Put your left finger at
...
(don\textquotesingle t forget it\textquotesingle s integer division!).
So put your nose at index 3 (or one of your toes): nose = 3
...
So let\textquotesingle s put our left finger at nose + 1, i.e. left =
nose + 1. We now have
...
Let\textquotesingle s repeat the process:
...
Now I\textquotesingle m ready to give you the code. This will set
\emph{index} to the index where the target is found. If it is not found
\emph{index} is set to -1. The name of the array is \emph{x} and the
size of the array is \emph{SIZE}.
...
scenario, it takes log\textsubscript{2}n. Here\textquotesingle s a table
of comparison:

n linear search (n) binary search (log\textsubscript{2}n)
...
between n and log\textsubscript{2}n widens. Specifically, by
l\textquotesingle Hôpital\textquotesingle s rule, the limit of
n/(log\textsubscript{2}n) as n approaches infinity is in fact
infinity.{]}
...
Here\textquotesingle s the pseudocode:
...
\vtop{\hbox{\strut }\hbox{\strut h e l}}
...
Here\textquotesingle s a hint. The code for drawing the tic-tac-toe
...
// print board{[}i{]} and \textquotesingle\textbar\textquotesingle{}
