...
\input{exercises/15-11-for-loops-part2/main.tex}A \EMPHASIZE{prime} is a positive integer which is greater than 1 and is
...
\input{exercises/16-11-for-loops-part2/main.tex}Note that it's obvious that, for instance when
...
\input{exercises/17-11-for-loops-part2/main.tex}But there's still something else you can do. Note that
...
std::cout << (isprime ? "prime" : "not prime")
          << std::endl;
\end{console}
\input{exercises/18-11-for-loops-part2/main.tex}There's yet another thing we can do. Note that \texttt{sqrtn} is used only in the for-loop. We can actually declare \texttt{sqrtn} at the initialization part of the for-loop so that the scope of \texttt{sqrtn} stays inside the loop:
\begin{console}[commandchars=\~\@\$]
bool isprime = true;
...
\input{exercises/19-11-for-loops-part2/main.tex}\input{exercises/20-11-for-loops-part2/main.tex}
